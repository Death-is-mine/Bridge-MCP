...
export function getMetadata(requestHost?: string) {
  const issuer = config.oauth.issuer !== "http://localhost:3000"
    ? config.oauth.issuer
    : requestHost || config.bridge.publicUrl;
  return {
    issuer,
...
export function getProtectedResourceMetadata(requestHost?: string) {
  const issuer = config.oauth.issuer !== "http://localhost:3000"
    ? config.oauth.issuer
    : requestHost || config.bridge.publicUrl;
  return {
